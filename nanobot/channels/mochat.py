...
class MochatChannel(BaseChannel):
    ...
    def _build_notify_handler(self, event_name: str):
        async def handler(payload: Any) -> None:
            if event_name == "notify:chat.inbox.append":
                ...
                await self._handle_notify_chat_message(payload, event_name)
        return handler
    ...
    async def _handle_notify_chat_message(self, payload: Any, event_name: str) -> None:
        if not isinstance(payload, dict):
            return
        group_id = _str_field(payload, "groupId")
        ...
        if event_name in {"notify:chat.message.recall", "notify:chat.message.delete"}:
            await self._dispatch_control_event(panel_id, "panel", payload, event_name)
            return
    ...
    async def _dispatch_control_event(
        ...
        """Forward recall/delete events as control messages to the agent loop."""
        author = _str_field(payload, "author", "messageAuthor")
        if not author or (self.config.agent_user_id and author == self.config.agent_user_id):
            return
        if not self.is_allowed(author):
            return
        control = "recall" if event_name.endswith(".recall") else "delete"
        await self._handle_message(
            sender_id=author,
            chat_id=target_id,
            content="",
