...
def test_onboard_fresh_install(mock_paths):
    ...
    assert "cookiebot is ready" in result.stdout
...
def test_agent_defaults_include_local_runtime_controls():
    config = Config()

    assert config.agents.defaults.bus_maxsize == 256
    assert config.agents.defaults.telemetry_enabled is True
    assert config.tools.exec.sandbox_backend == "sandboxd_docker"
    assert config.tools.exec.sandbox_profile == "workspace_write_no_net"
    assert config.tools.exec.docker_network_enabled is False
    assert config.tools.exec.docker_memory_mb == 512
    assert config.tools.exec.docker_pids_limit == 128
    assert config.tools.exec.docker_user == "65534:65534"
    assert config.tools.exec.docker_cpus == 1.0
    assert config.tools.exec.docker_seccomp_profile is None
...
def test_agent_config_sets_active_path(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("nanobot.bus.queue.MessageBus", lambda *args, **kwargs: object())
