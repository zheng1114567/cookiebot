...
@pytest.mark.asyncio
async def test_outbound_dispatch_times_out_stuck_channel_send():
    cancelled = asyncio.Event()

    class _SlowChannel(_FakePlugin):
        async def send(self, msg: OutboundMessage) -> None:
            try:
    ...
    mgr.config = cfg
    mgr.bus = bus
    mgr.channels = {"fakeplugin": _SlowChannel(SimpleNamespace(allow_from=["*"]), bus)}
    mgr._dispatch_task = None

    task = asyncio.create_task(mgr._dispatch_outbound())
    await bus.publish_outbound(OutboundMessage(channel="fakeplugin", chat_id="c1", content="hello"))
