...
def _make_tool_response(history_entry=None, memory_update=None, graph_nodes=None, graph_edges=None):
    """Create an LLMResponse with a save_memory tool call."""
    args: dict = {}
    if history_entry is not None:
        args["history_entry"] = history_entry
    if memory_update is not None:
        args["memory_update"] = memory_update
    if graph_nodes is not None:
        args["graph_nodes"] = graph_nodes
    if graph_edges is not None:
        args["graph_edges"] = graph_edges
    return LLMResponse(
        content=None,
        tool_calls=[
            ToolCallRequest(
...
class TestMemoryConsolidationTypeHandling:
    ...
    @pytest.mark.asyncio
    async def test_missing_history_entry_still_succeeds(self, tmp_path: Path) -> None:
        """history_entry is now optional — consolidation succeeds without it."""
        store = MemoryStore(tmp_path)
        provider = AsyncMock()
            ...
            return_value=LLMResponse(
                ...
                tool_calls=[
                    ToolCallRequest(
                        ...
                        arguments={
                            "graph_nodes": [
                                {"id": "topic:test", "type": "topic", "name": "Test", "summary": "A test node", "tags": ["test"]}
                            ],
                            "graph_edges": [],
                        },
                    )
        ...
        provider.embed = AsyncMock(return_value=None)  # no embedding
        messages = _make_messages(message_count=60)

        result = await store.consolidate(messages, provider, "test-model")

        assert result is True
        # Graph was created with the topic node
        assert store.graph.graph_file.exists()
        # History file is NOT created (no history_entry)
        assert not store.history_file.exists()

    @pytest.mark.asyncio
    async def test_missing_memory_update_still_succeeds(self, tmp_path: Path) -> None:
        """memory_update is now optional — consolidation succeeds without it."""
        store = MemoryStore(tmp_path)
        provider = AsyncMock()
        provider.chat_with_retry = AsyncMock(
            return_value=LLMResponse(
                ...
                tool_calls=[
                    ToolCallRequest(
                        ...
                        arguments={
                            "graph_nodes": [],
                            "graph_edges": [],
                            "history_entry": "[2026-01-01] Partial output.",
                        },
                    )
                ],
            )
        ...
        assert store.history_file.exists()
        # MEMORY.md was NOT written (no memory_update)
        assert not store.memory_file.exists()

    @pytest.mark.asyncio
    async def test_sensitive_memory_update_goes_to_secure_store(self, monkeypatch, tmp_path: Path) -> None:
                ...
                tool_calls=[
                    ToolCallRequest(
                        ...
                        arguments={
                            "graph_nodes": [],
                            "graph_edges": [],
                            "history_entry": "[2026-01-01] Sensitive credential discussed.",
                            "memory_update": "# Memory\nUser shared api_key sk-1234567890abcdef for staging.",
                        },
                    )
                ],
            )
        ...
        assert result is True
        assert not store.memory_file.exists()
        assert store.secure_memory_file.exists()
        secure = json.loads(store.secure_memory_file.read_text(encoding="utf-8"))
        assert secure["needs_user_confirmation"] is True

    @pytest.mark.asyncio
    async def test_project_name_adds_graph_hierarchy(self, tmp_path: Path) -> None:
        """Project-scoped graph nodes are automatically attached to a project container."""
        store = MemoryStore(tmp_path)
        provider = AsyncMock()
        provider.chat_with_retry = AsyncMock(
            return_value=LLMResponse(
                ...
                tool_calls=[
                    ToolCallRequest(
                        ...
                        arguments={
                            ...
                            "graph_nodes": [
                                {
                                    "id": "topic:memory-hierarchy",
                                    "type": "topic",
                                    "name": "Memory hierarchy",
                                    "summary": "Add hierarchical GraphRAG memory.",
                                    "tags": ["memory"],
                                }
                            ],
                            "graph_edges": [],
                            "history_entry": "[2026-01-01] Project memory hierarchy discussed.",
                        },
                    )
                ],
            )
        ...
        assert result is True
        data = json.loads(store.graph.graph_file.read_text(encoding="utf-8"))
        node_map = {node["id"]: node for node in data["nodes"]}
        child = node_map["topic:memory-hierarchy"]
        assert "project:nanobot" in node_map
        assert child["scope"] == "project"
        assert child["parent_id"] == "project:nanobot"
        assert ("project:nanobot", "topic:memory-hierarchy", "contains") in {
            (edge["source"], edge["target"], edge["type"]) for edge in data["edges"]
        }

    @pytest.mark.asyncio
    async def test_daily_category_adds_graph_hierarchy(self, tmp_path: Path) -> None:
        """Non-project graph nodes are grouped under a daily conversation category."""
        store = MemoryStore(tmp_path)
        provider = AsyncMock()
        provider.chat_with_retry = AsyncMock(
            return_value=LLMResponse(
                ...
                tool_calls=[
                    ToolCallRequest(
                        ...
                        name="save_memory",
                        arguments={
                            "daily_category": "planning",
                            "graph_nodes": [
                                {
                                    "id": "conversation:weekend-plan",
                                    "type": "conversation",
                                    "name": "Weekend plan",
                                    "summary": "User discussed weekend scheduling.",
                                    "tags": ["weekend"],
                                }
                            ],
                            "graph_edges": [],
                            "history_entry": "[2026-01-01] Weekend planning discussed.",
                        },
                    )
                ],
            )
        ...
        data = json.loads(store.graph.graph_file.read_text(encoding="utf-8"))
        node_map = {node["id"]: node for node in data["nodes"]}
        child = node_map["conversation:weekend-plan"]
        assert "daily:planning" in node_map
        assert child["scope"] == "daily"
        assert child["category"] == "planning"
        assert child["parent_id"] == "daily:planning"

    @pytest.mark.asyncio
    async def test_null_optional_fields_are_accepted(self, tmp_path: Path) -> None:
        """Null optional fields are silently skipped (no longer rejected)."""
        store = MemoryStore(tmp_path)
        provider = AsyncMock()
        provider.chat_with_retry = AsyncMock(
            return_value=_make_tool_response(
                history_entry=None,
                memory_update=None,
                graph_nodes=[{"id": "topic:x", "type": "topic", "name": "X", "summary": "Test", "tags": ["t"]}],
                graph_edges=[],
            )
        )
        provider.embed = AsyncMock(return_value=None)
        ...
        # Graph was created
        assert store.graph.graph_file.exists()
        # Neither history nor long-term were written (both null)
        assert not store.history_file.exists()
        assert not store.memory_file.exists()

    @pytest.mark.asyncio
    async def test_empty_history_entry_skipped_silently(self, tmp_path: Path) -> None:
        """Empty/whitespace history_entry is silently skipped."""
        store = MemoryStore(tmp_path)
        provider = AsyncMock()
        provider.chat_with_retry = AsyncMock(
            return_value=_make_tool_response(
        ...
        assert result is True
        # Whitespace-only history_entry is skipped
        assert not store.history_file.exists()
