...
def test_fetch_new_messages_polls_configured_accounts(monkeypatch) -> None:
    raw = _make_raw_email(subject="Account message", body="hello")

    class FakeIMAP:
        def __init__(self, host: str, _port: int) -> None:
            self.host = host
            self.username = ""
        ...
        def fetch(self, _imap_id: bytes, _parts: str):
            uid = b"101" if "qq" in self.username else b"202"
    ...
    monkeypatch.setattr("nanobot.channels.email.imaplib.IMAP4_SSL", FakeIMAP)
    ...
    channel = EmailChannel(
        {
            ...
            "accounts": {
                "qq": {
                    "imapHost": "imap.qq.example",
                    "imapUsername": "bot@qq.example",
                    "imapPassword": "secret",
                    "smtpHost": "smtp.qq.example",
                    "smtpUsername": "bot@qq.example",
                    "smtpPassword": "secret",
                },
                "163": {
                    "imapHost": "imap.163.example",
                    "imapUsername": "bot@163.example",
                    "imapPassword": "secret",
                    "smtpHost": "smtp.163.example",
                    "smtpUsername": "bot@163.example",
                    "smtpPassword": "secret",
                },
            },
        },
    ...
    assert {item["account"] for item in items} == {"qq", "163"}
    assert all(item["metadata"]["account"] in {"qq", "163"} for item in items)
    assert all("Account:" in item["content"] for item in items)


def test_disabled_email_accounts_are_skipped() -> None:
    channel = EmailChannel(
        {
            "enabled": True,
            "consentGranted": True,
    ...
    assert sorted(channel.accounts) == ["163"]
...
@pytest.mark.asyncio
async def test_send_reply_uses_last_inbound_account(monkeypatch) -> None:
    class FakeSMTP:
    ...
    channel._last_account_by_chat["alice@example.com"] = "163"

    await channel.send(
        OutboundMessage(
            channel="email",
            chat_id="alice@example.com",
            content="Reply from matching account.",
        )
    )

    assert len(fake_instances) == 1
    assert fake_instances[0].host == "smtp.163.example"
    assert fake_instances[0].sent_messages[0]["From"] == "bot@163.example"
