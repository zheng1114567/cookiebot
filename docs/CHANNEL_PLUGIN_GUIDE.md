...
Build a custom cookiebot channel in three steps: subclass, package, install.

## How It Works

cookiebot discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `cookiebot gateway` starts, it scans:
...
```bash
pip install -e .
cookiebot plugins list    # verify "Webhook" shows as "plugin"
cookiebot onboard         # auto-adds default config for detected plugins
```
...
| `default_config()` (classmethod) | Returns default config dict for `cookiebot onboard`. Override to declare your fields. |
...
Override `default_config()` so `cookiebot onboard` auto-populates `config.json`:
...
pip install -e .
cookiebot plugins list  # should show "Webhook" as "plugin"
cookiebot gateway       # test end-to-end
```

## Verify

```bash
$ cookiebot plugins list
